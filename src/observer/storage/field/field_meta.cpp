...
FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool nullable, int id)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, nullable, id);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool nullable, int id)
{
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  if (!id_value.isInt()) {
    LOG_ERROR("Len is not an integer. json value=%s", id_value.toStyledString().c_str());
    return RC::INTERNAL;
  }
  ...
  return field.init(name, type, offset, len, visible, nullable, id);
}
