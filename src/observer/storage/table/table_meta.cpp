...
RC TableMeta::init(int32_t table_id, const char *name, int field_num, const AttrInfoSqlNode attributes[])
{
  ...
  const int extra_field_num = 1;  // used for null value
  if (trx_fields != nullptr) {
    fields_.resize(field_num + trx_fields->size() + extra_field_num);

    for (size_t i = 0; i < trx_fields->size(); i++) {
      const FieldMeta &field_meta = (*trx_fields)[i];
      fields_[i] = FieldMeta(field_meta.name(), field_meta.type(), field_offset, field_meta.len(), false/*visible*/, field_meta.nullable(), i);
      field_offset += field_meta.len();
    }

    trx_field_num = static_cast<int>(trx_fields->size());
  } else {
    fields_.resize(field_num + extra_field_num);
  }

  for (int i = 0; i < field_num; i++) {
    const AttrInfoSqlNode &attr_info = attributes[i];
    rc = fields_[i + trx_field_num].init(attr_info.name.c_str(), 
            attr_info.type, field_offset, attr_info.length, true/*visible*/, attr_info.nullable, i + trx_field_num);
    if (rc != RC::SUCCESS) {
  ...
  // put null field end of record
  size_t null_field_len = (field_num - 1) / 8 + 1;  // the length contain sys fields
  rc = fields_[field_num + trx_field_num].init("__null", CHARS, field_offset, null_field_len, false, false, field_num + trx_field_num);
  if (RC::SUCCESS != rc) {
...
FieldMeta *TableMeta::null_bitmap_field()
{
  return &fields_.back();
}
