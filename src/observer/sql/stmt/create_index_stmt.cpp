...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if(create_index.attribute_name.size() == 0){
    const char *table_name = create_index.relation_name.c_str();
    LOG_WARN("invalid argument. db=%p, table_name=%p, index name=%s, attribute num=0",
          db, table_name, create_index.index_name.c_str());
    return RC::INVALID_ARGUMENT;
  }

  if(create_index.attribute_name.size() == 1){
    const char *table_name = create_index.relation_name.c_str();
    ...
    FieldMeta *field_meta = table->table_meta().field(create_index.attribute_name[0].c_str());
    ...
    for (int i = 0; i < create_index.attribute_name.size(); i++){
      FieldMeta *field_meta = table->table_meta().field(create_index.attribute_name[i].c_str());
      if (nullptr == field_meta) {
        LOG_WARN("no such field in table. db=%s, table=%s, field name=%s", 
