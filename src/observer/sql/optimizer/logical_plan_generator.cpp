...
RC LogicalPlanGenerator::create_plan(
    SelectAggStmt *select_agg_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  unique_ptr<LogicalOperator> table_oper(nullptr);
  ...
  unique_ptr<LogicalOperator> predicate_oper;
  RC rc = create_plan(select_agg_stmt->filter_stmt(), predicate_oper);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to create predicate logical plan. rc=%s", strrc(rc));
    return rc;
  }
    ...
    SelectJoinStmt *select_join_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<LogicalOperator> predicate_oper;
  RC rc = create_plan(select_join_stmt->filter_stmt(), select_join_stmt->filter_stmt_join(), predicate_oper);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to create predicate logical plan. rc=%s", strrc(rc));
    return rc;
  }
...
RC LogicalPlanGenerator::create_plan(FilterStmt *filter_stmt, FilterStmt *filter_stmt_join, std::unique_ptr<LogicalOperator> &logical_operator){
  std::vector<unique_ptr<Expression>> cmp_exprs;
  const std::vector<FilterUnit *> &filter_units = filter_stmt->filter_units();
  const std::vector<FilterUnit *> &filter_units_join = filter_stmt_join->filter_units();
  for (const FilterUnit *filter_unit : filter_units_join) {
    const FilterObj &filter_obj_left = filter_unit->left();
    const FilterObj &filter_obj_right = filter_unit->right();
    ...
    UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(table, update_stmt->values(), update_stmt->attribute_name()));
  unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(table, update_stmt->exprs(), update_stmt->fields_, update_stmt->attribute_name()));
