...
RC PhysicalPlanGenerator::create(LogicalOperator &logical_operator, unique_ptr<PhysicalOperator> &oper)
{
  ...
  switch (logical_operator.type()) {
    ...
    case LogicalOperatorType::PROJECTION_AGG: {
      return create_plan(static_cast<ProjectAggLogicalOperator &>(logical_operator), oper);
    } break;
...
RC PhysicalPlanGenerator::create_plan(TableGetLogicalOperator &table_get_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  for (auto &expr : predicates) {
    if (expr->type() == ExprType::COMPARISON) {
      ...
      if (value_expr->value_.attr_type() == NULLS){
        index = nullptr;
        break;
      }
      ...
      index = table->find_index_by_field(field.field_name(), loc);
      loc--;
      if (nullptr == index) {
        break;
      }
  ...
  if (index != nullptr) {
    // printf("index name: %s\n", static_cast<BplusTreeIndex *>(index)->index_meta().name());
...
RC PhysicalPlanGenerator::create_plan(ProjectAggLogicalOperator &project_oper, unique_ptr<PhysicalOperator> &oper)
{
  vector<unique_ptr<LogicalOperator>> &child_opers = project_oper.children();
  ...
  LOG_TRACE("create a project agg physical operator");
  return rc;
}
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(), update_oper.exprs(), update_oper.fields_, update_oper.attribute_name()));
