...
struct OrderByNode
{
  RelAttrSqlNode order_by_attr;  // order by attribute
...
struct InsertSqlNode
{
  ...
  std::vector<std::vector<Value>> values;         ///< 要插入的值，可能同时插入多行
};
...
struct CreateIndexSqlNode
{
  ...
  std::string relation_name;    ///< Relation name
  std::vector<std::string> attribute_name;  /// Attribute name, To support multi-index
  bool unique;                  ///< unique index or not
};
